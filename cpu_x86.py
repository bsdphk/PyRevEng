...
class x86(object):
	...
	def modRM(self, p):
		...
		if mod == 0:
			...
			if x & 0x80:
				x |= 0xffffff00
			f = "0x%02x"
		elif mod == 2 and self.asz == 16:
			x = p.m.w16(self.na)
			self.na += 2
			f = "0x%04x"
		elif mod == 2 and self.asz == 32:
			x = p.m.w32(self.na)
			self.na += 4
			f = "0x%08x"
		else:
	...
	def sfrm(self, p, mne, args, suff=False, wid=0):
		...
		if suff:
			if self.mrm != None and self.mrm[0] == 3:
				pass
			elif args[0] == "Sw":
				self.mne += "w"
	...
	def __decode(self, p):
		...
		if iw in shortform:
			...
			if self.syntax == "intel":
				...
				self.sfrm(p, "movzb", ("Gv", "Eb"), True)
		elif 0x0fb7 == iw:
			self.modRM(p)
			if self.syntax == "intel":
				self.sfrm(p, "movzx", ("Gv", "Eb"))
			else:
				self.sfrm(p, "movzw", ("Gv", "Ew"), True)
		elif 0x0fba == iw:
			self.modRM(p)
			if self.mrm[1] > 3:
				self.sfrm(p,
